  ...
  const overlays = useMemo(() => {
    ...
    return layout.rooms.map(room => {
      ...
      if (activeHeatmap === 'sunlight') {
        const score = analysisResult.naturalLighting.rooms?.find(r => r.roomId === room.id)?.score || 0;
        ...
        const sunScore = analysisResult.naturalLighting.rooms?.find(r => r.roomId === room.id)?.score || 0;
